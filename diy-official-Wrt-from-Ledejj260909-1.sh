...
#
# 从LedeJJ环境经测试改造而来配置。因官方openwrt的源码目录没有lede源码额外加的自己特色的目录，因此必须去掉哪些对于没有目录部分的操作  2026.9.17新增该文件，供调用
...
git_sparse_clone main https://github.com/kenzok8/small lua-neturl                            #kenzok8 的small-package中无此插件，在small中 # 核查有260909
git_sparse_clone main https://github.com/kenzok8/small redsocks2                             #kenzok8 的small-package中无此插件，在small中 # 核查有260909
git_sparse_clone main https://github.com/kenzok8/small shadow-tls                            #kenzok8 的small-package中无此插件，在small中 # 核查有260909
...
# 修改默认登录地址
#sed -i 's/192\.168\.[0-9]*\.[0-9]*/192.168.4.10/g' ./package/base-files/files/bin/config_generate                                  #000000有这个文件，但命令没有生效

#修改openwrt.lan关联IP
#sed -i "s/192\.168\.[0-9]*\.[0-9]*/192.168.4.10/g" $(find ./feeds/luci/modules/luci-mod-system/ -type f -name "flash.js")          #000000有这个flash.js文件但没有ip地址字段
     
#2. 修改默认登录密码
#sed -i 's/$1$V4UetPzk$CYXluq4wUazHjmCDBCqXF.//g' ./package/lean/default-settings/files/zzz-default-settings                        #000000没lean这个目录
...
# TTYD 免登录
#sed -i 's|/bin/login|/bin/login -f root|g' feeds/packages/utils/ttyd/files/ttyd.config                                               #000000该命令这个动作时点没有生效
...
#sed -i '/<tr><td width="33%"><%:CPU usage (%)%><\/td><td id="cpuusage">-<\/td><\/tr>/a <tr><td width="33%"><%:Github Project%><\/td><td><a href="https:\/\/github.com\/YingziWo\/Actions-OpenWrt-build" target="_blank">Actions Build System<\/a><\/td><\/tr>' ./package/lean/autocore/files/x86/index.htm              #000000无这个目录

# 修改本地时间格式
#sed -i 's/os.date()/os.date("%a %Y-%m-%d %H:%M:%S")/g' package/lean/autocore/files/*/index.htm                                         #000000无lean这个目录

#修改镜像源
#sed -i 's#mirror.iscas.ac.cn/kernel.org#mirrors.edge.kernel.org/pub#' scripts/download.pl                                              #000000官方文件中无mirror.iscas.ac.cn字段

# 修改版本为编译日期
date_version=$(date +"%y.%m.%d")
#orig_version=$(cat "package/lean/default-settings/files/zzz-default-settings" | grep DISTRIB_REVISION= | awk -F "'" '{print $2}')      #000000无lean这个目录
#sed -i "s/${orig_version}/R${date_version} by Yingziwo/g" package/lean/default-settings/files/zzz-default-settings
#sed -i "s/${orig_version}/R${date_version} by Yingziwo/g" package/lean/default-settings/files/zzz-default-settings                     #000000无lean这个目录

#删除无效opkg源
#sed -i '/exit 0/i sed -i "/kenzok8/d" /etc/opkg/distfeeds.conf' ./package/lean/default-settings/files/zzz-default-settings              #000000无lean这个目录
#sed -i '/exit 0/i sed -i "/kenzo/d" /etc/opkg/distfeeds.conf' ./package/lean/default-settings/files/zzz-default-settings                #000000无lean这个目录
#sed -i '/exit 0/i sed -i "/small/d" /etc/opkg/distfeeds.conf' ./package/lean/default-settings/files/zzz-default-settings                #000000无lean这个目录

#删除多余文件
#sed -i '/exit 0/i\rm -f /etc/config/adguardhome\nrm -f /etc/init.d/adguardhome' ./package/lean/default-settings/files/zzz-default-settings   #000000无lean这个目录
